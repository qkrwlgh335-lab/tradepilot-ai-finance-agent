...
외화 거래의 현금흐름 위험을 계산하고, 공식 출처가 연결된 자격 규칙으로 상담 후보를
좁힌 뒤 근거·반사실 시나리오·상담 브리프 초안을 제공하는 로컬 웹 프로토타입입니다.
제8회 KB AI Challenge 주제 6번 출품을 위해 개발했습니다.

> 이 저장소는 포트폴리오용 비공식 프로토타입이며 KB국민은행·한국무역보험공사의
> 공식 서비스가 아닙니다. 금융기관의 내부 승인모형이 아닌 **1차 스크리닝 모델**로,
...
### 3. 규칙 판정과 공식 근거가 연결된 상담 후보

![상담 후보와 공식 근거](docs/screenshots/03-product-evidence.png)
...
- **자격은 규칙이 판정합니다.** 필수 정보가 없으면 적격으로 추정하지 않고
  `정보 필요`로 보류합니다.
- **RAG는 판정 뒤에만 동작합니다.** 이미 추론된 후보 범위에서 공식 근거를 찾는
...
| 검증 | 공개 시점 결과 | 해석 범위 |
|---|---:|---|
| 자동 테스트 | 676/676 통과 | 계산, 규칙, UI 계약, RAG, 프록시, 문서 등 회귀 테스트 |
...
- 상품 조건과 공식 출처 범위가 제한되어 있으며, 실제 적용에는 상품 마스터·모델 검증·
...
정리했습니다. 이 프로젝트는 생성형 AI를 코드 초안, 테스트 아이디어, 문서 검토에
